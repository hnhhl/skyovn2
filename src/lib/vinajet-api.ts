    ...
    try {
      ...
      const sendProgressUpdate = (immediate = false) => {
        ...
        if (immediate) {
          ...
          // Debounce updates to prevent flickering - tăng timeout để stable hơn
          if (updateTimer) clearTimeout(updateTimer)
      ...
      const searchPromises = this.defaultAirlines.map(async (airline) => {
        ...
        while (retryCount <= maxRetries) {
          try {
            ...
            // Chỉ send update cho airline đầu tiên để tránh flicker
            ...
            if (result.status && result.departure && result.departure.length > 0) {
              ...
              // Chỉ send update immediate khi có results mới
              sendProgressUpdate(true)
            } else {
              // Không có results thì debounce update
              sendProgressUpdate()
            }
