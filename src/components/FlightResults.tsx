    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filteredAndSortedFlights.map((flight, index) => {
          ...
          return (
            <Card 
             key={flight.detailId ? `flight-${flight.detailId}` : `flight-${index}-${segment.flightCode}-${segment.startDate}-${flight.flightNumber}`}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-2">
                        {airlineFlights.map((flight, idx) => {
                          const segment = flight.segments?.[0]
                         const compKey = flight.detailId ? `comp-${flight.detailId}` : `comp-${airline.code}-${idx}-${segment?.flightCode || 'unknown'}`
