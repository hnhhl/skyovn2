    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {filteredAndSortedFlights.map((flight, index) => {
          ...
          // FIXED: Create unique key using detailId or fallback with more unique identifiers
          const flightKey = flight.detailId ? `flight-${flight.detailId}` : `flight-${index}-${segment.flightCode}-${segment.startDate}-${flight.flightNumber}-${segment.airline}`
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-2">
                        {airlineFlights.map((flight, idx) => {
                          const segment = flight.segments?.[0]
                          // FIXED: Create unique key for comparison modal
                          const compKey = flight.detailId ? `comp-${flight.detailId}` : `comp-${airline.code}-${idx}-${segment?.flightCode || 'unknown'}-${segment?.startDate || Date.now()}`
                          
                          if (!segment) return null

                          return (
                            <div key={compKey} className="p-2 bg-gray-50 rounded text-sm">
