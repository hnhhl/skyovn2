...
export function FlightResults(props: FlightResultsProps) {
  ...
  const [displayMode, setDisplayMode] = useState<'loading' | 'results' | 'no-results'>('loading')
              ...
              <table className="min-w-[540px] w-full border-2 border-white text-sm text-center bg-white rounded-lg shadow-sm overflow-hidden">
                ...
                <tbody>
                  {['Sáng', 'Trưa', 'Chiều', 'Tối', 'Đêm'].map(period => {
                    ...
                    return (
                      <Card key={`${airline}-${flight.flightValue || flight.flightNumber}-${flightIndex}`} className={`cursor-pointer transition-all duration-200 hover:shadow-md border ${
                    ...
                    <TabsContent key={`compare-tab-${airline}`} value={airline} className="mt-4">

                          const cheapest = periodFlights.reduce((min, cur) =>
                            cur.info.totalPrice < min.info.totalPrice ? cur : min, periodFlights[0]
                            <Card key={`compare-flight-${airline}-${flight.flightValue || flight.flightNumber}-${idx}`} className="p-4 hover:shadow-md transition-shadow">
...
🌟 Liên hệ để được tư vấn thêm!`
  ...
  // Update display mode based on progressive status
  useEffect(() => {
    if (!progressive) {
    ...
    const hasData = progressive.combinedResults?.departure?.length > 0
    const isComplete = progressive.status === 'complete'

    if (hasData && displayMode !== 'results') {
      setDisplayMode('results')
      setIsInitialLoad(false)
    } else if (isComplete && !hasData && displayMode !== 'no-results') {
      setDisplayMode('no-results')
      setIsInitialLoad(false)
    } else if (!hasData && !isComplete && displayMode !== 'loading') {
      setDisplayMode('loading')
    }
  }, [progressive?.status, progressive?.combinedResults?.departure?.length, displayMode])
